...
export default function InputPro({ disabled, label, option, align }: Props) {
  ...
  let props = ['fl-input', disabled ? 'is-disable' : ''];
  // let option=['']
  console.log(option);
  ...
  let ffocus = () => {
    ...
    for (let i = 0; i < inps.length; i++) {
      inps[i].onfocus = function () {
        x = i;
        // console.log(x);
        let divs: any = document.querySelectorAll('.fl-input--optDiv');
  ...
  let wz: any = weizhi(align);
  console.log(wz);

  return (
    <div className="fl-input--div" style={{ position: 'relative' }}>
      <input className={props.join(' ')} onFocus={ffocus} onBlur={blurr} placeholder="请输入" />
      ...
      <div className="fl-input--optDiv" style={wz}>
        {option.map((item: any, index: any) => {
          return (
            <option
              className={item.disabled ? '' : 'fl-input--option'}
              onClick={() => chac(item.label)}
              key={index}
